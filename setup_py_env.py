...
def ensure_pip(a: Automata, PR: Path) -> None:
    pe = pyexe(PR)
    a.run_string(f"{pe} -s -m ensurepip --default-pip --upgrade --verbose")


def upgrade_pip(a: Automata, PR: Path) -> None:
    pe = pyexe(PR)
...
def make_venv(a: Automata, PR: Path, VRdir: Path) -> None:
    venvexe = PR / bindir() / f"virtualenv{ext()}"
    pyexe = PR / bindir() / f"python{ext()}"
    a.run_string(
        f"{venvexe} {VRdir} --python={pyexe} --verbose --always-copy --clear"
    )
...
def activate_venv(a: Automata, VR: Path) -> None:
...
def install_pkgs(
    a: Automata, pkglist: typing.List[str], PR: Path, do_upgrade: bool = True
) -> None:
    pe = pyexe(PR)
    pkgs = " ".join(pkglist)
...
def install_ports(
    a: Automata, portlist: typing.List[str], PR: Path = None
) -> None:
    drive = "i:/" if is_nt() else "/i"
...
def install_virtualenv(a: Automata, PR: Path = None) -> None:
    pkglist = [
        "virtualenv",
    ]
...
def install_our_pkgs(a: Automata, PR: Path = None) -> None:
...
def copy_python_exe(a: Automata, PR: Path) -> None:
...
def copy_ext_dlls(a: Automata, PR: Path) -> None:
...
def do_setup(a: Automata, PR: Path) -> None:
